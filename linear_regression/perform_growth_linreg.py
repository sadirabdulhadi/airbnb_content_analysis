...
pref= "median-more40_"
file_root = "/Users/Sadir/Documents/Computer_science/Year4/term2/individual_project/results/london/dictionary-results/variations/"
saving_file = "/Users/Sadir/Documents/Computer_science/Year4/term2/individual_project/results/london/total-linreg/variation/"
...
def clean_dataframe(census, reviews, year):
    ...
    sf.rename(columns={"metric_change": 'index'}, inplace = True)
    global keys_df
...
def perform_linreg(table_reviews, table_census, final_saving_file, metric):
    ...
    for key in pvalues["metric_change"].keys():
        row_pvalues[key] = []
        row_coef[key] = []
    ...
    for key in pvalues["metric_change"]:
...
for dictionary in dictionaries:
    for prop_type in types:
        for metric in metrics[dictionary]:
            print(dictionary, prop_type, metric)
            print("normal")
            table_reviews = file_root + dictionary+"/"+prop_type+"/"+prop_type + "variations" + metric
            final_saving_file = saving_file +dictionary + "/" + prop_type + "/" + metric
            perform_linreg(table_reviews, table_census, final_saving_file, metric)
            print("stepwise")
            final_saving_file = saving_file + "stepwise-"+dictionary + "/" + prop_type + "/" + metric
